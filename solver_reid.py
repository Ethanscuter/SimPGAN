...
from evaluate import evaluate_idt
...
class Solver():
    ...
    def build_model(self):
        ...
        self.dreid = DReid(class_count=self.num_classes_market)
    ...
    def train(self):
        ...
        for step in range(self.train_iters + 1):
            ...
            if step >= 500:
                # Reid loss G21(A) -> fake A
                self.reset_grad()
                fake_market = self.g12(market)
                feature, out = self.dreid(fake_market)
                dreid_loss = criterion(out, m_labels)

                # Reid loss G21(G12(A)) -> fake A
                fake_grid = self.g21(market)
                fake_market = self.g12(fake_grid)
                feature, out = self.dreid(fake_market)
                dreid_loss += criterion(out, m_labels)

                # Reid loss real image
                feature, out = self.dreid(market)
                dreid_loss += criterion(out, m_labels)
